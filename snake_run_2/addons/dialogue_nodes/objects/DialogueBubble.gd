...
func _process(delta):
	...
	if follow_node is Node2D:
		...
		camera_manual = get_node("/root/Node/GridContainer/SubViewportContainer2/SubViewport/Player/rotation_helper/Camera3D")
		screen_center = get_viewport_rect().size * 0.5
		follow_pos = camera_manual.unproject_position(follow_node.global_position)
